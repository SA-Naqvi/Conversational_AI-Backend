...
def build_prompt(
    ...
    Args:
        ...
        state_guidance: Stage-specific guidance text.
        rag_context: Formatted retrieved document context (from RAG retriever).
        crm_context: Formatted CRM user profile context.
    """
    ...
    # Append CRM context (returning user info)
    if crm_context:
        system_content += f"\n\n{crm_context}\n"
    ...
    if rag_context:
        system_content += (
            f"\n\nYou have access to the following retrieved medical knowledge relevant "
            f"to the patient's question. Use this information to ground your response. "
            f"You may cite the source documents when appropriate.\n{rag_context}\n"
        )
